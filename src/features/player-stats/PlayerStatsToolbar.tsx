...
const PLAYER_STATS_TAB_ORDER: readonly TabKey[] = ['allgemein', 'items', 'mobs'];

export function getPlayerStatsTabId(tab: TabKey): string {
  return `player-stats-tab-${tab}`;
}

export function getPlayerStatsPanelId(tab: TabKey): string {
  return `player-stats-panel-${tab}`;
}
...
export function PlayerStatsToolbar({
  ...
  const handleTabKeyDown = (event: KeyboardEvent<HTMLButtonElement>, index: number): void => {
    if (event.key === 'ArrowRight' || event.key === 'ArrowDown') {
      ...
      focusTabAtIndex((index + 1) % PLAYER_STATS_TAB_ORDER.length);
      return;
    }

    if (event.key === 'ArrowLeft' || event.key === 'ArrowUp') {
      event.preventDefault();
      focusTabAtIndex((index - 1 + PLAYER_STATS_TAB_ORDER.length) % PLAYER_STATS_TAB_ORDER.length);
      return;
    }
    ...
    if (event.key === 'End') {
      event.preventDefault();
      focusTabAtIndex(PLAYER_STATS_TAB_ORDER.length - 1);
    }
  };
