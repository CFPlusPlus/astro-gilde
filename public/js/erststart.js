...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function computeActiveIdx() {
        ...
        for (let i = 0; i < sections.length; i++) {
            ...
            if (top - threshold <= 0) idx = i; else break;
        }
    ...
    function updateActiveByScroll() {
        if (isProgrammaticScroll) {           // während Klick-Scroll: nicht umschalten
    ...
    function layoutSteps() {
        ...
        // 4) Anwenden
        steps.forEach((li, idx) => { li.style.top = adjusted[idx] + "px"; });
    }

    // --- Initialisierung / Reflows ---
    window.addEventListener("resize", () => { layoutSteps(); updateActiveByScroll(); });
    window.addEventListener("load", () => { layoutSteps(); updateActiveByScroll(); });
    layoutSteps();
    updateActiveByScroll();

    // Falls Schrift/Icon später lädt → kurz nachtriggern
    setTimeout(() => { layoutSteps(); updateActiveByScroll(); }, 300);
