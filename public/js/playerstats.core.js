(function () {
    // --- UUID-Utils ---
    function compactUUID(s) {            // "550e8400-e29b..." -> "550e8400e29b..."
        return String(s || "").toLowerCase().replace(/[^0-9a-f]/g, "");
    }
    function prettyUUID(c32) {           // "550e8400e29b41d4a716446655440000" -> mit Bindestrichen
    ...
    function nf(x) { return new Intl.NumberFormat('de-DE').format(x); }
    function nf2(x) { return new Intl.NumberFormat('de-DE', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(x); }
    ...
    function checkMissingTranslations(stats) {
        ...
        // Items
        const itemSections = [
            "minecraft:used", "minecraft:mined", "minecraft:crafted",
            "minecraft:dropped", "minecraft:picked_up", "minecraft:broken", "minecraft:placed"
        ];
    ...
    async function loadAll() {
        ...
        const res = await fetch(`api/player?uuid=${encodeURIComponent(uuidParam)}`, { cache: "no-store" });
        if (!res.ok) throw new Error("HTTP " + res.status);
        ...
        buildGeneral(stats); buildItems(stats); buildMobs(stats);
    }
    ...
    function buildGeneral(stats) {
        ...
        const rows = Object.entries(custom).map(([raw, value]) => {
    ...
    function buildItems(stats) {
        const tbody = document.querySelector('#tbl-items tbody'); tbody.innerHTML = '';
        ...
        const keys = new Set([...Object.keys(mined), ...Object.keys(broken), ...Object.keys(crafted), ...Object.keys(used), ...Object.keys(picked), ...Object.keys(dropped)]);
        ...
        for (const r of rows) {
            const tr = h('tr', {}, h('td', {}, r.label), h('td', {}, nf(r.mined)), h('td', {}, nf(r.broken)), h('td', {}, nf(r.crafted)), h('td', {}, nf(r.used)), h('td', {}, nf(r.picked_up)), h('td', {}, nf(r.dropped)));
            tbody.appendChild(tr);
        }
    }

    function buildMobs(stats) {
        const tbody = document.querySelector('#tbl-mobs tbody'); tbody.innerHTML = '';
        ...
        const rows = [...keys].map(k => ({ key: k, label: tLabel(k, 'mob'), killed: killed[k] || 0, killed_by: killedBy[k] || 0 })).sort((a, b) => a.label.localeCompare(b.label, 'de'));
        for (const r of rows) {
            const tr = h('tr', {}, h('td', {}, r.label), h('td', {}, nf(r.killed)), h('td', {}, nf(r.killed_by)));
            tbody.appendChild(tr);
        }
    }
    ...
    (function () {
        ...
        const NUMERIC_KEYS = new Set([
            "value", "mined", "broken", "crafted", "used", "picked_up", "dropped", "killed", "killed_by"
        ]);
        ...
        function applySort(table, colIndex, state) {
            ...
            if (state === "none") {
                ...
                sorted = rows.sort((r1, r2) =>
                    r1.cells[0].textContent.trim()
                        .localeCompare(r2.cells[0].textContent.trim(), "de")
                );
            } else {
    ...
    (function () {
        ...
        uuidEl.addEventListener("click", () => {
            ...
            navigator.clipboard.writeText(uuid).then(() => {
                ...
                setTimeout(() => { uuidEl.textContent = oldText; }, 1200);
            }).catch(err => {
