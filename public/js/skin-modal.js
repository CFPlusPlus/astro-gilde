...
export async function openSkinModal() {
    ...
    const isLowPowerMode = typeof navigator !== "undefined" && "connection" in navigator
        ? (navigator.connection.saveData || (navigator.connection.effectiveType || "").includes("2g"))
        : false;

    let canvas = null;
    let viewer = null, controls = null;
    let ro = null, rafId = 0, dragRafId = 0;
    let isAlive = false, isDragging = false;
    ...
    function onCtxLost(e) { e.preventDefault(); setStatus("WebGL-Kontext verloren."); }

    // --- Utils ------------------------------------------------------------------
    function ensureSkinviewReady() {
        return new Promise((resolve, reject) => {
            if (window.skinview3d) { resolve(window.skinview3d); return; }
    ...
    function getQueryParam(name) { try { return new URL(window.location.href).searchParams.get(name) || ""; } catch { return ""; } }
    function normalizeUUID(u) { const c = (u || "").replace(/[^0-9a-fA-F]/g, "").toLowerCase(); return c.length === 32 ? c : ""; }
    ...
    async function fetchMojangTextures(uuid) {
        ...
        try { return JSON.parse(atob(texProp.value)); } catch { return null; }
    }
    ...
    function createFreshCanvas() {
        try { wrap.querySelector("#skin-canvas")?.remove(); } catch { }
    ...
    function fitCanvas(c) {
        ...
        if (!w || !h) { w = 600 * dprCap; h = 600 * dprCap; }
    ...
    function startDragLoop() {
        ...
        if (dragRafId) return;
        const loop = () => {
            if (!isAlive || !isDragging) { dragRafId = 0; scheduleDraw(); return; }
            scheduleDraw();
    ...
    function stopDragLoop() {
        try { if (controls) { const sel = document.getElementById('anim-select'); controls.enabled = (sel && sel.value === 'none'); } } catch { }
        isDragging = false;
    ...
    function startAnimLoop() {
        ...
        if (animRafId) { pendingResize = true; return; }
        ...
        setTimeout(() => {
            if (typeof canvas !== "undefined" && canvas && typeof viewer !== "undefined" && viewer) {
                try { fitCanvas(canvas); } catch { }
                try { viewer.setSize(canvas.width, canvas.height); } catch { }
            }
        }, 0);
        let lastTs;
        ...
        const loop = (ts) => {
            if (!isAlive || !viewer) { animRafId = 0; return; }
            ...
            /* Draw + adaptives DPR (fixed) wird unten eingefüg */ try {
                ...
                if (typeof lastTs !== "number") { lastTs = ts; }
                const dt = Math.max(0, ts - lastTs);
                lastTs = ts;
                if (typeof window.__fpsAccT === "undefined") { window.__fpsAccT = 0; window.__fpsAccN = 0; }
                window.__fpsAccT += dt; window.__fpsAccN++;

                // Ausstehendes Resize einmal innerhalb der Schleife anwenden, um Thrashing zu vermeiden
                if (pendingResize && typeof canvas !== "undefined" && canvas && typeof viewer !== "undefined" && viewer) {
                    pendingResize = false;
                    try { fitCanvas(canvas); viewer.setSize(canvas.width, canvas.height); } catch { }
                }
                ...
                frameAccMs += dt;
                if (frameAccMs < targetMs) { /* In diesem Tick den aufwändigen Draw überspringen */ } else {
                    frameAccMs = frameAccMs % targetMs;
                    try { controls?.update?.(); } catch { }
                    viewer.draw && viewer.draw();
                }

                if (window.__fpsAccT >= 2000) { // Alle ~2s
                    const fps = window.__fpsAccN * 1000 / window.__fpsAccT;
                    window.__fpsAccT = 0; window.__fpsAccN = 0;

                    const curr = (typeof dprOverride === 'number' ? dprOverride : (isCoarse ? 1 : Math.min(window.devicePixelRatio || 1, 1.5)));
                    let next = curr;
                    if (fps < 50) next = Math.max(0.9, curr - 0.1); // herunterstufen
    ...
    function attachControlEvents(ctrl) {
        ...
        ctrl.addEventListener?.("start", () => { isDragging = true; startDragLoop(); }, { passive: true });
        ...
        if (typeof wrap !== "undefined" && wrap && !wrapPointerBound) {
            const onDown = () => { isDragging = true; startDragLoop(); };
            ...
            removeWrapPointerEvents = () => {
                try { wrap.removeEventListener("pointerdown", onDown); } catch { }
                try { wrap.removeEventListener("pointerup", onUp); } catch { }
                try { wrap.removeEventListener("pointercancel", onUp); } catch { }
    ...
    function stopAllAnimations(opts = {}) {
        ...
        try { if (viewer) viewer.animation = null; } catch { }
        try { if (viewer) viewer.autoRotate = false; } catch { }
    ...
    function applyAnimation(mode) {
            ...
            case "fly":
                ...
                try {
                    if (viewer?.playerObject) {
                        ...
                        if (prevBackEquipment == null) prevBackEquipment = viewer.playerObject.backEquipment || "cape";
        ...
        if (hasAnim) startAnimLoop(); else stopAnimLoop();
        scheduleDraw();
    ...
    function hardReleaseGL() {
        try {
            stopAllAnimations();
            cancelLoop();
            try { viewer && (viewer.draw = function () { }); } catch { }
            ...
            try { canvas?.removeEventListener("webglcontextlost", onCtxLost, { passive: false }); } catch { }
    ...
    function openModal() {
        ...
        if (titleEl) {
            ...
            titleEl.textContent = ident.name
                ? `Skin von ${ident.name}`
                : (shortUuid ? `Skin ${shortUuid}` : "Spielerskin");
        }
        ...
        if (hdr) { hdr.style.display = "flex"; void hdr.offsetHeight; }

        // Canvas/Wiederzeichnung sofort anstoßen
        try { dialogEl.focus({ preventScroll: true }); } catch { }
    ...
    function bindResetHandler() {
        ...
        btnReset._resetHandler = () => {
            ...
            try {
                ...
                controls?.reset?.();
                if ("zoom" in viewer) {
                    viewer.zoom = pose.zoom; viewer.fov = 55;
                    viewer.camera.updateProjectionMatrix?.();
                }
                try { viewer.playerObject && viewer.playerObject.rotation.set(0, 0, 0); } catch { }
                ...
                controls?.update?.();
                scheduleDraw();
            } catch (e) { console.warn("Reset fehlgeschlagen:", e); }
        };
    ...
    function closeModal() {
        ...
        try { removeWrapPointerEvents && removeWrapPointerEvents(); } catch { }
        try { if (onVisChange) document.removeEventListener("visibilitychange", onVisChange); } catch { }
        onVisChange = null;
        ...
        try { window.removeEventListener("resize", onWinResize, { passive: true }); } catch { }
    ...
    async function initViewer() {
        ...
        if (titleEl) {
            const shortUuid = ident.uuid ? ident.uuid.slice(0, 8) + "…" : "";
            titleEl.textContent = ident.name ? `Skin von ${ident.name}` : (shortUuid ? `Skin ${shortUuid}` : "Spielerskin");
        }

        const CY = 12;
        viewer = new skinview3d.SkinViewer({ canvas, width: canvas.width, height: canvas.height, zoom: 0.9 });
        ...
        } catch (e) { console.warn("Controls/Kamera:", e); }
        ...
        } catch (e) { console.warn("Animations init:", e); }

        // 1) Steve vorladen
        setStatus("Initialisiere Renderer …");
        try { await viewer.loadSkin("https://minotar.net/skin/MHF_Steve", { model: "auto" }); } catch { }
        ...
        if (capeLoaded) {
            try { viewer.playerObject.backEquipment = "cape"; } catch { }
            setStatus("");
            scheduleDraw();
        } else {
        ...
        function setupAnimationUI() {
            ...
            if (!hint) {
                const candidate = Array.from(footer.children)
                    .find(el => /Ziehen\/zoomen|ESC/i.test((el.textContent || "")));
            ...
            select._animHandler = () => {
                ...
                try { localStorage.setItem("skinAnimMode", select.value); } catch { }
            };
            ...
            try { document.fonts?.ready?.then(updateLabelWidth); } catch { }
        ...
        try {
            ro = new ResizeObserver((entries) => {
                ...
                const dpr = (typeof dprOverride === 'number' ? dprOverride : (isCoarse ? 1 : Math.min(window.devicePixelRatio || 1, 2)));
        ...
        onVisChange = () => {
            if (document.hidden) {
                try { stopAllAnimations && stopAllAnimations(); } catch { }
                try { cancelLoop && cancelLoop(); } catch { }
            } else {
